...
TM_FATFS_GetDriveSize(char* str, TM_FATFS_Size_t* SizeStruct) {
    ...
    res = f_getfree((TCHAR*)str, &fre_clust, &fs);
    if (res != FR_OK) {
        return res;
    }
...
TM_FATFS_DriveSize(uint32_t* total, uint32_t* free) {
    ...
    /* Get volume information and free clusters of drive */
    res = f_getfree((TCHAR*)"0:", &fre_clust, &fs);
    if (res != FR_OK) {
        return res;
    }
...
TM_FATFS_USBDriveSize(uint32_t* total, uint32_t* free) {
    ...
    /* Get volume information and free clusters of drive */
    res = f_getfree((TCHAR*)"1:", &fre_clust, &fs);
    if (res != FR_OK) {
        return res;
    }
...
scan_files(char* path, uint16_t tmp_buffer_size, TM_FATFS_Search_t* FindStructure) {
    ...
    /* Try to open file */
    if ((res = f_opendir(&dir, (TCHAR*)path)) == FR_OK) {
        ...
        while ((res = f_readdir(&dir, &fno)) == FR_OK && fno.fname[0] != 0) {
            ...
            /* Format name */
#if _USE_LFN
            fn = (char*)(*fno.lfname ? fno.lfname : fno.fname);
#else
            fn = fno.fname;
#endif
